...
### Added
- **A task can now declare the compute class it needs — `ms.WithCompute(ms.Heavy(ms.Res{VCPU: 4, MemoryMB: 8192}))`.** Tasks execute on the module's *own* Lambda, so `OnTask` has always inherited Lambda's 15-minute ceiling and modest vCPU allocation. That is not a missing abstraction, it is a missing execution tier: a four-rendition encode of one source hour needs roughly ninety minutes on 4 vCPU and no API shape makes it fit. A task may now name `ms.Standard` (Lambda, the default), `ms.Heavy` (Fargate) or `ms.GPU` (ECS on EC2 — Fargate has no GPU support at all, so it is a materially different runner, not a flag), and the request rides the manifest at `tasks[].compute` so the platform provisions the right runner at deploy from the declaration alone — the same mechanism that already provisions the SQS queue. **The default does not move**: a task registered without `WithCompute` produces a byte-identical manifest entry and keeps running exactly where it ran before, so no module lands on expensive compute because the SDK grew a field. A class is a backend selector and the module sizes it, which also makes the request *visible at install and therefore approved by the app owner* — the same trust root that already governs dependencies and exposed tables — rather than hidden behind an opaque tier name.
- **The sizing rules are enforced at declaration, not at deploy.** A resource request that only fails during a deploy is the worst of both worlds, so `Standard`/`Heavy`/`GPU` validate eagerly and panic at startup like `ms.ExposeTable`, naming both the offending value and the rule that rejected it. `VCPU` on `Standard` is a panic rather than a silent no-op because Lambda *derives* vCPU from memory and cannot honour it. `Heavy` is checked against Fargate's real CPU/memory matrix, which is not free-form — `ms.Heavy(ms.Res{VCPU: 0.5, MemoryMB: 512})` is rejected, because 512 MB is legal at 0.25 vCPU and nowhere else. `GPU` takes an instance from a platform allowlist (the NVENC-capable `g4dn` and `g5` sizes available in ap-northeast-1) and rejects `VCPU`/`MemoryMB` alongside it: `g4dn.xlarge` *is* 4 vCPU / 16 GB, so accepting both would invite a contradiction the platform would have to silently resolve. The matrix and the allowlist are held as data and the panic messages are rendered from that same data, so the rule quoted to the author cannot drift from the rule enforced.
