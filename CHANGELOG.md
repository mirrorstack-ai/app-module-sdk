...
### Added
- **`ms.ExposeTable(name, readableBy ...string)`** — declare a relation in this module's `mod_<id>` schema (table, view, or materialized view — `GRANT SELECT` applies uniformly, so the SDK doesn't distinguish) as readable by the listed consumer modules. The catalog translates each declaration into a Postgres `GRANT SELECT` against the consumer's DB role at install time.
  Pair with the consumer's `ms.DependsOn("@<owner>/<this>.<name>")` — both sides must match for the read to actually happen. This is the SDK side of Phase 2's data-routing story (DB proxy enforcement lands separately on the platform).
  `readableBy` entries are exact `@<owner>/<module>` references — no wildcards. Matching by module *name* across owners (`@*/analytics`) would only be meaningful with a platform-level module-spec system; without one, every consumer is listed explicitly so the GRANT surface stays auditable from the source.
  **Dedup semantics**: declaring the same name twice merges the `readableBy` slices (set union, preserving order). A second call can never silently drop a reader the first call established — feature-flagged additions compose safely.
