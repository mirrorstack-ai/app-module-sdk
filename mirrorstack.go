...
// readable by the listed `@<owner>/<module>` consumers (exact references —
// no wildcards). The catalog translates each entry into a Postgres GRANT
// at install time. Pair with the consumer's `ms.DependsOn`. See the core
