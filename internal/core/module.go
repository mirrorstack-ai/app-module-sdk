...
// alphanumerics/hyphens, max 16 chars. The 16-char cap exists so the worst-case
// constructed identifier `<username>_<slug>_<table>_<col>_fkey` fits inside
// Postgres's 63-byte NAMEDATALEN ceiling (see docs/module-identity-and-storage-prefix.md
// "Postgres identifier budget"). The CLI's publish-time linter is the real
// gate against overflow; this regex catches the obvious shape errors at
// New() time so a malformed slug doesn't reach the manifest.
