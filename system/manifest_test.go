...
func TestManifest_SlugSurfaced(t *testing.T) {
	...
	// The catalog reads the manifest's slug to anchor the human-readable
	// half of @<owner>/<slug> and to seed the storage prefix. Round-tripping
	// it here pins the wire shape — a future change that drops the field
	// would silently break catalog ingest.
...
func TestManifest_SlugOmittedWhenEmpty(t *testing.T) {
	...
	// Dev/legacy modules that haven't been assigned a slug yet should
	// produce a manifest without the "slug" key — the platform reads
	// presence/absence to fall back to ID-based addressing for unpublished
	// modules. omitempty in the struct tag is the contract.
	req := httptest.NewRequest("GET", "/__mirrorstack/platform/manifest", nil)
	rec := httptest.NewRecorder()
...
func TestManifest_EmptyEventsAndSchedules_NotNull(t *testing.T) {
	...
	for _, want := range []string{`"emits":[]`, `"subscribes":{}`, `"schedules":[]`, `"versions":{}`, `"permissions":[]`, `"exposures":[]`, `"migration":{"app":""}`} {
